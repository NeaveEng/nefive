...
def configureDepthPostProcessing(stereo):
    """
...
leftCam.initialControl.setFrameSyncMode(dai.CameraControl.FrameSyncMode.INPUT)
leftOut = pipeline.create(dai.node.XLinkOut)
leftOut.setStreamName("left")
queueNames.append("left")

rightCam = pipeline.create(dai.node.ColorCamera)
rightCam.initialControl.setFrameSyncMode(dai.CameraControl.FrameSyncMode.INPUT)
...
# config.postProcessing.thresholdFilter.minRange = 120
# config.postProcessing.thresholdFilter.maxRange = 1500
...
left_img_pub = rospy.Publisher('camera/left/image/raw', Image, queue_size=1)
right_img_pub = rospy.Publisher('camera/right/image/raw', Image, queue_size=1)
center_img_pub = rospy.Publisher('camera/center/image/raw', Image, queue_size=1)
depth_img_pub = rospy.Publisher('camera/depth/image/raw', Image, queue_size=1)
disparity_img_pub = rospy.Publisher('camera/disparity/image/raw', Image, queue_size=1)
